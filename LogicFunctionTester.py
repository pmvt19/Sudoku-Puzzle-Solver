...
def filterBads():
    runningPossibleValues = copy.deepcopy(possibleValues)

    for i in range(9):
        for j in range(9):
            print(len(runningPossibleValues[i][j])== 2 , end = " ")
            print(board[i][j], end = " ")
            if(len(runningPossibleValues[i][j])== 2 ):
                print("YEET", end = " ")
                arrayOfBools = []
                board[i][j] = runningPossibleValues[i][j][0]
                resolution()
                if(subbingConditions()):
                    arrayOfBools.append(True)
                else:
                    arrayOfBools.append(False)
                board[i][j] = runningPossibleValues[i][j][1]
                resolution()
                if (subbingConditions()):
                    arrayOfBools.append(True)
                else:
